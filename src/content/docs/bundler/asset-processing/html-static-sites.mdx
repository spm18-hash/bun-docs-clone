...
```html label="index.html" icon="file-code"
<!doctype html>
<html>
  <head>
...
<Aside>Bun's transpiler is also used at runtime.</Aside>
...
```toml label="bunfig.toml" icon="settings"
[serve.static]
...
<Tabs>
  ...
  <TabItem label="styles.css">
    ...
    ```css label="styles.css" icon="file-code"
    @import "tailwindcss";
    ```
  ...
  <TabItem label="app.ts">
    ...
    ```ts label="app.ts" icon="/icons/typescript.svg"
    import "tailwindcss";
    ```
...
<Aside>Only one of those are necessary, not all three.</Aside>
...
<Tabs>
  ...
  <TabItem label="API">
    ```ts label="build.ts" icon="/icons/typescript.svg"
    // Inline all environment variables
    await Bun.build({
...
```js label="dist/app.js" icon="/icons/javascript.svg"
...
<Aside>
  Currently, plugins are only supported through `Bun.build`'s API or through
  `bunfig.toml` with the frontend dev server - not yet supported in `bun
  build`'s CLI.
</Aside>
...
<Aside>You've never seen a watch mode this fast.</Aside>
